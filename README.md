...
这里收录可安装、可审计、带确定性脚本的 Codex Skills。目前包括 GitHub 项目发布与 Release、
准备 CSDN 技术文章发布包，以及制作网页项目横竖版宣传视频四条工作流。
...
| [`github-project-publisher`](github-project-publisher/) | 一体化提交、推送、版本 Tag、源码归档、SHA-256、GitHub Release 和远端校验 | V1 |
...
https://github.com/OWNER/REPOSITORY，并在 dry-run 通过后提交、推送和校验远端结果。
如果指定版本，再创建 Tag、源码归档、SHA-256 和 GitHub Release。
```
...
`github-safe-publish` 和 `github-project-publisher` 必须获得明确目标和推送授权；另外两个 Skill 只生成本地文件，不会
登录、编辑、上传或发布到第三方平台。
...
`github-project-publisher` 在一个可审计流程中完成：

- 本地候选文件、敏感信息和 GitHub 100 MiB 限制检查；
- 目标仓库和远端历史检查；
- 提交、普通 fast-forward push 和远端 SHA 校验；
- 从版本号或项目清单生成 `vX.Y.Z` Tag；
- `git archive` 源码 ZIP、Release Notes、显式资产和 `SHA256SUMS.txt`；
- 通过已认证的 GitHub CLI 创建 Release，并独立核对 Release 资产。

默认命令是 dry-run。分支发布需要 `--execute`；创建 Release 还需要 `--release`、版本号和已认证的 `gh` CLI。它不会创建 GitHub 仓库，也不会覆盖已有 Tag 或使用 force push。
...
python C:\path\to\skill-creator\scripts\quick_validate.py github-project-publisher
...
`github-project-publisher` 的 Release 功能要求用户已有 GitHub CLI 登录状态；Skill 不读取或
输出 Token。Release 资产目录默认写在项目外，并保留机器可读 receipt 供复核。
